...
class BootstrapProjectKernelTests(unittest.TestCase):
    ...
    def test_copy_templates_apply_writes_files(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Issue Sync", (target / "docs" / "PRD_TEMPLATE.md").read_text(encoding="utf-8"))
            self.assertIn("updated", (target / "docs" / "PRD_TEMPLATE.md").read_text(encoding="utf-8"))
            self.assertIn("skipped", (target / "docs" / "PRD_TEMPLATE.md").read_text(encoding="utf-8"))
            self.assertIn("not_applicable", (target / "docs" / "PRD_TEMPLATE.md").read_text(encoding="utf-8"))
            self.assertIn("Issue Sync", (target / "AGENTS.md").read_text(encoding="utf-8"))
            self.assertIn("Issue Sync", (target / "CONTRIBUTING.md").read_text(encoding="utf-8"))
            self.assertIn("Work Item Routing", (target / "docs" / "PRD_TEMPLATE.md").read_text(encoding="utf-8"))
            self.assertIn("Project Health Audit", (target / "docs" / "PRD_TEMPLATE.md").read_text(encoding="utf-8"))
            self.assertIn("Optional Weekly Operating Loop", (target / "docs" / "PRD_TEMPLATE.md").read_text(encoding="utf-8"))
            self.assertIn("Work Item Routing", (target / "AGENTS.md").read_text(encoding="utf-8"))
            self.assertIn("Project Health Audit", (target / "AGENTS.md").read_text(encoding="utf-8"))
            self.assertIn("Optional Weekly Operating Loop", (target / "CONTRIBUTING.md").read_text(encoding="utf-8"))
